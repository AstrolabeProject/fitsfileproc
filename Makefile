...
.PHONY: help build clean cleanout exec reset run-bash run-mnt run-db run-db-q watch

help:
	@echo 'Make what? build, clean, cleanout, docker, exec, jar, macjar, reset,'
	@echo '           run-bash, run-db, run-db-q, run-mnt, testjar-mnt, testjar-db, watch'
	@echo '  where:'
...
run-mnt:
	docker run -it --rm --name ${NAME} -v ${IMGS}:/images -v ${OUTDIR}:/out ${IMG} --verbose -of sql
